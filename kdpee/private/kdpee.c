...
/* stdbool.h stuff */
	...
	#define true 1
	#define bool _Bool
	#if __STDC_VERSION < 199901L && __GNUC__ < 3
		typedef int _Bool;
	#endif
#endif
...
floatval kdpee(const floatval **dimrefs, const int n, const int d, floatval *mins, 
	...
	minlev = ceil(0.5 * log2(n)); /* Min partitioning level */
	for(i = 0; i < n; ++i){
...
floatval kdpee_recurse(const floatval **dimrefs, const int n, const int d, floatval *mins, 
	...
	int dimno, thesize, i, newmaxindex, newminindex;
	floatval median, zscore, frac, volume, oldextremum, left, right;
	
	dimno = curlev % d; /*  The dimension along which we're intending to split */
	thesize = 1+maxindex-minindex; /*  No of points in this subset */
	
	/*  As well as returning the median, this PARTITIONS the data (keys) in-place */
	median = kdpee_hoareMedian(dimrefs[dimno], keys, minindex, maxindex);
