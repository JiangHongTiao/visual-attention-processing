function ittiSaliencyMap_video(inVid,outVid,savFlg,demoFlg,inDat)
...
M = 3;
queue = zeros([frame_size 3 M]);
w = 64; % Windowsize
params = defaultSaliencyParams;
params.oriComputeMode = 'full'; % Use full computational mode since there is error with efficient mode in UCSD database
%%
...
while ~isDone(hmfr)        
    ...
    imgrgb = step(hmfr); % Convert color image to intensity   
    imgstr = initializeImage(uint8(imgrgb));
    % Detecting Lane-mark by saliency method
    [smraw,~] = makeSaliencyMap(imgstr,params);
    smraw = imresize(smraw.data,frame_size,'bilinear');
    ...
    if (demoFlg == 1)
        step(hvideo1, imgrgb);        % Display Original Video
