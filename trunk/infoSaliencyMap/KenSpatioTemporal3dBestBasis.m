...
if repOpt == 1
    ...
    resFld = ['./results/KenSpatioTemporal3dBestBasis-' inFld '-top' num2str(numOfTopPoints) '-' datestr(now,'yyyymmddTHHMMSS') '/']; % Linux result folder  
end
if (savFlg == 1)
    mkdir(resFld);
end
...
%             patch     = patch./norm(patch(:));
            if dwtSpaceFlg == 1 
                ...
                if BestBasisFlg == 1
                    ...
                    % output: DwtScore;
                    SpatialDwtScoreArray((row_index+PatchSize-1)/PatchSize,(col_index+PatchSize-1)/PatchSize,frame_index) = BestBasis3d(patch,numLevels);
                else
