function SVAEM_Extraction(inVid,savFlg,demoFlg,params)
...
if (params.saliencyMethods(5) == 1)
    ...
    hbfr5 = video.BinaryFileReader(...
                    'Filename',['./results/autosplit/infoSaliencyMaps_date-20100828T151403/' inVid '.bin'], ...
                    'VideoFormat','Custom', ...
                    'BitstreamFormat','Planar', ...
                    'VideoComponentCount',1, ...
                    'VideoComponentSizes',[544 720], ...
...
data = zeros(noFrms,6);
salValAtEyeMarks = dataset({data,'ITTI','GBVS','PFT','PQFT','INFO','ENTRO'},'ObsNames',nameFrms);
%%
...
%             yLocMin = yLoc - 20; yLocMax = yLoc + 20;
            if (yLoc - 20 < 1) yLocMin = 1; yLocMax = 41;
            elseif (yLoc + 20 > frame_size(1)) yLocMax = frame_size(1); yLocMin = yLocMax - 40;               
            else yLocMin = yLoc - 20; yLocMax = yLoc + 20;
            end           
            
            if (xLoc - 20 < 1) xLocMin = 1; 
            elseif (xLoc + 20 > frame_size(2)) xLocMax = frame_size(2);
            else xLocMin = xLoc - 20; xLocMax = xLoc + 20;
            end
            
            salValAtEyeMarks(iFrame,i) = dataset(max(max(img(yLocMin:yLocMax,xLocMin:xLocMax))));
%             salValAtEyeMarks(iFrame,i) = dataset(img(yLoc,xLoc));
            if (demoFlg == 1)
...
if (savFlg == 1)
    ...
    curFld = pwd;
    cd(params.savePath);
    save([inVid '.mat'],'salValAtEyeMarks');
    cd(curFld);
end
%% Summary
