...
display('Testing Cayley-Dickson functions (cd, dc, cdpolar) ...')
...
check(dc(i,0) == qi,            'dc function fails test 3');
check(dc(0,1) == qj,            'dc function fails test 4');
check(dc(0,i) == qk,            'dc function fails test 5');
...
[A, B] = cd(qi);
check(A == i && B == 0, 'cd function fails test 8');

[A, B] = cd(qj);
check(A == 0 && B == 1, 'cd function fails test 9');

[A, B] = cd(qk);
check(A == 0 && B == i, 'cd function fails test 10');
...
[A, B] = cdpolar(quaternion(0));
compare(A, 0, T, 'cdpolar function fails test 12a');
compare(B, 0, T, 'cdpolar function fails test 12b');

[A, B] = cdpolar(quaternion(1));
compare(A, 1, T, 'cdpolar function fails test 13a');
compare(B, 0, T, 'cdpolar function fails test 13b');

[A, B] = cdpolar(qi);
compare(A, i, T, 'cdpolar function fails test 14a');
compare(B, 0, T, 'cdpolar function fails test 14b');

[A, B] = cdpolar(qj);
compare(A,    1, T, 'cdpolar function fails test 15a');
compare(B, pi/2, T, 'cdpolar function fails test 15b');

[A, B] = cdpolar(qk);
compare(A,        1, T, 'cdpolar function fails test 16a');
compare(B, i * pi/2, T, 'cdpolar function fails test 16b');
...
Q = quaternion(randn(50), randn(50), randn(50), randn(50));

cdtest(Q, T,'28');

% Repeat the previous test with random sign changes in order to test the
% second (logical) parameter to cdpolar. (This causes some elements of A to
% be negated, but since the corresponding element of B will be changed to
% compensate, the result should still reconstruct correctly).

[A, B] = cdpolar(Q, randn(size(Q)) > 0); % Random logical array.
compare(Q, dc(A) .* exp(dc(B) .* qj), T, 'cdpolar function fails test 29');
...
% $Id: test_cd.m,v 1.3 2009/02/08 19:18:16 sangwine Exp $
