function salValAtMaximumPoint = SVAMP_Extraction(inVid,savFlg,demoFlg,params)
...
disp(['Extract maximum saliency points in ' inVid '.avi']); 
...
inDat(inDat == 0) = 1;
salValAtMaximumPoint = dataset({data,'ITTI','GBVS','PFT','PQFT','INFO','ENTRO'},'ObsNames',nameFrms);
%%
...
while ~isDone(hbfr1)        
    ...
    for i = 1:1:6
        if (params.saliencyMethods(i) == 1)
            ...
            salValAtMaximumPoint(iFrame,i) = dataset(max(max(img)));
            if (demoFlg == 1)
...
if (savFlg == 1)   
    ...
    curFld = pwd;
    cd(params.savePath);
    save([inVid '.mat'],'salValAtMaximumPoint');
    cd(curFld);
end
%% Summary
