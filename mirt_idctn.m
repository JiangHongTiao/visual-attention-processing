...
if precompute,
    ...
    for i=1:ndim,
        n=siz(i); clear tmp;
        if rem(n,2)==1 || ~isreala, % odd case or complex
            
            ww{i} = sqrt(2*n)*exp(1i*(0:n-1)'*pi/2/n);
            ww{i}(1) = ww{i}(1)/sqrt(2);
            
            tmp(1:2:n)=(1:ceil(n/2)); tmp(2:2:n)=(n:-1:ceil(n/2)+1);
            ind{i}=bsxfun(@plus, tmp', 0:n:n*(prod(siz)/n-1));
                         
        else % real even case (faster)
            ww{i} = sqrt(2*n) * exp((1i*pi/(2*n))*(0:n-1)');
            ww{i}(1) = ww{i}(1)/sqrt(2);
       
            tmp(1:2:n)=(1:n/2); tmp(2:2:n)=(n:-1:n/2+1);
            ind{i}=bsxfun(@plus, tmp', 0:n:n*(prod(siz)/n-1));
        end
