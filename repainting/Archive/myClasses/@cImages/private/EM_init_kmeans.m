...
Mu = Centers';
for i=1:nbStates
  idtmp = find(Data_id==i); % Collect all data in the same i cluster
  Priors(i) = length(idtmp); % Count how many they are
  Sigma(:,:,i) = cov([Data(:,idtmp) Data(:,idtmp)]'); % Calculate the square convariance of data points in the same cluster
