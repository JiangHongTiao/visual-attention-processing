...
%     im_name                     = ['fb-',num2str(frame_index+29),'.jpg'];
...
for frame_index = 4:NumFrames
    ...
    for row_index = 1:PatchSize:RowFrames
        ...
        for col_index = 1:PatchSize:ColFrames
            ...
            for layer_index = 1:1:4
                patch(:,:,layer_index)     = patch(:,:,layer_index)./(norm(patch(:,:,layer_index)));                                
            end
            
            im_patch  = patch(:); 
            
            for layer_index = 1:1:4
                patch(:,:,layer_index)     = dct2(patch(:,:,layer_index));
            end
...
%             dct_patch = dct_patch(:);  % now can convert to one-dimensional vector
    ...
    title('Dct Score');
%     current_folder = pwd;
%     cd('./results/KenSpatioTemporal3d/');
%     saveas(frame_index,['frame_' num2str(frame_index,'%03d') '.jpg']);
%     cd(current_folder);
end;
